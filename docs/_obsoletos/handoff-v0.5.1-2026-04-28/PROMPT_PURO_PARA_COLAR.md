> ⚠️ **SUPERSEDED em 2026-04-29.** Este prompt orientava a v0.5.1 cirúrgica via openpyxl no template antigo. Foi superado pelo caminho **DADOS+visual** (ver `HANDOFF_PLANILHA_DADOS_VS_VISUAL.md` + `RELATORIO_MIGRACAO_DADOS.md`). O template já está migrado. Para o trabalho residual no CLI (log_writer append + smoke 30 parcelas + testes dual-mode), use `PROMPT_CLI_RESIDUAL.md`. Mantido aqui só por contexto histórico.
...
> Commits: <hash1> template, <hash2> código, <hash3> testes.
> Tag: v0.5.1 → <hash>.
